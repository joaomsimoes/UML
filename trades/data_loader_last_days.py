...
START_SPOT = "2022-05-01"
START_FUTURES = "2022-05-01"
END = "2022-05-30"
COIN = 'BTCUSDT'
...
def download_files(type=None):
    ...
    if type == 'futures':
        ...
        for date in date_list:
            file = f"https://data.binance.vision/data/futures/um/daily/trades/BTCUSDT/BTCUSDT-trades-{date.strftime('%Y-%m-%d')}.zip"
            url.append(file)
    else:
        date_list = pd.date_range(start=START_SPOT, end=END)
        for date in date_list:
            file = f"https://data.binance.vision/data/{type}/daily/trades/{COIN}/{COIN}-trades-{date.strftime('%Y-%m-%d')}.zip"
            url.append(file)
